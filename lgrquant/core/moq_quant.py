...
# def round_gptq(W, scale_out, zero_out, sym, min_bound, max_bound, H00=None,blocksize=128, **kwargs):
#     newW = W.clone()
#     Q = W.clone()
...
def round_gptq(W, scale_out, zero_out, sym, min_bound, max_bound, H00=None, blocksize=128, groupsize=-1, **kwargs):
    """
    GPTQ量化函数，支持组批次更新
    ...
    if H.isnan().any():
        raise ValueError("Hessian矩阵包含NaN值，无法继续量化")
    ...
    for i1 in range(0, W.shape[1], blocksize):
        ...
        for i in range(count):
            ...
            # 量化当前列
            q = my_round(safe_div(w - zero_out[:, i1 + i], scale_out[:, i1 + i]))
            q = q.clamp(min=min_bound, max=max_bound)
            q = q * scale_out[:, i1 + i] + zero_out[:, i1 + i]
            q = q.flatten()
