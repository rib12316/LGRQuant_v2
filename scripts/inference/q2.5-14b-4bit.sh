#!/usr/bin/env bash
# Qwen2.5-14B 4-bit (stage1 true_quant) 真量化推理
set -e
...
LOG_FILE="${SCRIPT_DIR}/../../logs/q2.5-14b-4bit_$(date +%Y%m%d_%H%M%S).log"
mkdir -p "$(dirname "$LOG_FILE")"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "=========================================="
echo "LGRQuant Inference: Qwen2.5-14B W4A16"
echo "=========================================="
echo "Model:      /data2/llms/qwen-2.5-14b"
echo "Checkpoint: /data2/xx_llms/LGRQuant_fast/qwen-2.5-14b/w4/stage1/true_quant.pth"
echo "Start:      $(date)"
echo "Log:        $LOG_FILE"
echo ""
...
python -m lgrquant.inference.inference_test \
    ...
    --true_quant_path /data2/xx_llms/LGRQuant_fast/qwen-2.5-14b/w4/stage1/true_quant.pth \
    --group_size -1 \
    --prompt "who are you?" \
    --max_new_tokens 128 \
    --temperature 0.7 \
...
python "$SCRIPT_DIR/generate_summary.py" "$LOG_FILE" "Qwen2.5-14B" "W4A16"
